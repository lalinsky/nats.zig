...
test "pending_msgs counter sync subscription" {
    var nc = nats.Connection.init(testing.allocator, .{});
    defer nc.deinit();
    try nc.connect("nats://localhost:4222");

    // Create sync subscription
    const sub = try nc.subscribeSync("test.pending.sync");
    defer sub.deinit();

    // Initially should be 0
    try testing.expect(sub.pending_msgs.load(.acquire) == 0);
    try testing.expect(sub.pending_bytes.load(.acquire) == 0);

    // Publish a message
    const msg1_data = "test message 1";
    try nc.publish("test.pending.sync", msg1_data);
    try nc.flush();
    ...
    try testing.expect(sub.pending_msgs.load(.acquire) == 1);
    try testing.expect(sub.pending_bytes.load(.acquire) == msg1_data.len);

    // Publish another
    const msg2_data = "test message 2";
    try nc.publish("test.pending.sync", msg2_data);
    try nc.flush();
    std.time.sleep(10 * std.time.ns_per_ms);

    // Should have 2 pending messages and correct total bytes
    try testing.expect(sub.pending_msgs.load(.acquire) == 2);
    try testing.expect(sub.pending_bytes.load(.acquire) == msg1_data.len + msg2_data.len);
    ...
    try testing.expect(sub.pending_msgs.load(.acquire) == 1);
    try testing.expect(sub.pending_bytes.load(.acquire) == msg2_data.len);
...
test "pending_msgs counter async subscription" {
    ...
    const sub = try nc.subscribe("test.pending.async", testHandler, .{TestContext{
    ...
    try nc.publish("test.pending.async", msg1_data);
    try nc.publish("test.pending.async", msg2_data);
    try nc.publish("test.pending.async", msg3_data);
    try nc.flush();
    ...
    try testing.expect(processed_count == 3);
    ...
    try testing.expect(total_bytes_processed == expected_bytes);
}
